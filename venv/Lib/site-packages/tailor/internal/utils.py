...
# This module is used for utility functions used throughout the library
# Consider to place these elsewhere in the future
...
def get_default_project():
    try:
        ...
        raise ValueError(
            'No default project specified in config, '
            'please provide a project name')


def get_project_data(project_name: Optional[str] = None):
    """

    """

    # Use default project if project=None, if no default project raise Exception
    if project_name is None:
        project_name = get_default_project()

    # look for locally defined projects first
    try:
        prj_data = config.PROJECTS.get(project_name)
    except AttributeError:  # no PROJECTS in config
        prj_data = None

    # look for projects defined in Tailor account
    # NOT IMPLEMENTED

    # still no luck
    if prj_data is None:
        raise ProjectError(f'Could not find project "{project_name}"')
